...
def Run_Funding_Routine(Funding_Set, FX_data, security_database, folder):
    '''
    Method that initially runs cost minimisation model for given Funding_Set:
        - LpProblem object is created.
        - LpVariable matrices (decision variables) are created.
        - LpAffineExpression (objective function) is created and assigned to the problem.
        - LpConstraints are created and assigned to the problem. These contraints include:
            - RQV cover
            - longbox cover
            - ISIN usage
            - priority securities utilisation and avoidance of allocation to longbox
            - holiday securities utilisation
            - concentration limits
        - Solver algorithm is chosen and applied to the problem.
        - Problem status is returned reflecting feasibility/optimality/well-definedness.
    If cost minimisation problem is feasible:
        - Result matrices (allocation and longbox) are populated.
        - Result matrix entry aggregation (security usage/margin cover).
        - Allocation details are exported.
        - Action files (detailing required security moves) are exported.
    If cost minimisation model is infeasible (ie. because of insufficient inventory), cover maximisation model is run for given Funding_Set:
        - LpProblem object is created.
        - LpVariable matrix (decision variables) is created.
        - LpAffineExpression (objective function) is created and assigned to the problem.
        - LpConstraints are created and assigned to the problem. These contraints include:
            - RQV cap
            - ISIN usage
        ...
        - Solver algorithm is chosen and applied to the problem.
        - Problem status is returned reflecting feasibility/optimality/well-definedness.
    If cover maximisation problem is feasible:
        - Result matrix (allocation) is populated.
        - Result matrix entry aggregation (security usage/margin cover) is performed.
        - Unallocated collateral details are exported.
        - Predicted short is determined.
    '''

    # Cost minimisation model is created.

    cost_model = Model(name='CostMinimisation', sense=1, Funding_Set=Funding_Set)
    ...
    cost_model.Assign_LpConstraint_Concentration_Limits(FX_data)
    ...
    if cost_model.problem.status == 1:
        ...
        cost_model.Constraint_Satisfaction_Check_RQV_Cover(folder)
        cost_model.Constraint_Satisfaction_Check_ISIN_Usage(folder)
        cost_model.Constraint_Satisfaction_Check_Concentration_Limits(FX_data, folder)

        # Allocation details are exported.

        cost_model.Export_Allocation_Details(folder)

        # Action files are exported.

        cost_model.Export_Action_Files(folder)
        ...
        cover_model = Model(name='CoverMaximisation', sense=0, Funding_Set=Funding_Set)
        ...
        cover_model.Assign_LpConstraint_Concentration_Limits(FX_data)
        ...
        if cover_model.problem.status == 1:
            ...
            cover_model.Insufficient_Cover_Analysis(folder)
